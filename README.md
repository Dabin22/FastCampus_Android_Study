...
## 39. ThreadBasic_Tetris small project
 > 쓰레드를 활용하여 테트리스를 만들어보는 프로젝트입니다.
   ###1. 2016.10.17~21 업데이트 -> 화면 구성,프리뷰,블록 낙하 후 맵에 충돌시 맵에 종속, 블록 이동 및 변환 기능
   ###2. 2016.10.24 업데이트 ->꽉채우면 줄사라지고 위의 줄이 내려오고 점수는 나옵니다. 죽지를 않습니다.
   ###3. 2016.10.25 -> 엔드버튼이나 앱 종료시 블록 쓰레드도 같이 종료되게 하였습니다. 현재 앱이 일시 중지시 블록이 멈추었다가 앱이 재시작시 다시 블록이 내려가기 시작하는 기능 구현중...
